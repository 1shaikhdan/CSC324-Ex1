...
(define (num-evens numbers)
  (cond
    ...
    [else (let ([first-number (first numbers)]
                [rest-num-evens (num-evens (rest numbers))])
            (if (equal? (modulo first-number 2) 0)
                (+ 1 rest-num-evens)
                rest-num-evens ))]))
...
(define (num-many-evens lists-of-numbers)
  (cond
    ...
    [else (let ([first-list (first lists-of-numbers)]
                [rest-list-evens (num-many-evens (rest lists-of-numbers))])
            (if (>= (num-evens first-list) 3)
                (+ 1 rest-list-evens)
                rest-list-evens )
            )
          ]
...
(module+ test
  ...
  (test-equal? "num-many-evens: simple non-empty list"
               ...
               1)
  (test-equal? "num-many-evens: multi-non-empty list"
               (num-many-evens (list (list 2 4 5 7 8) (list 2 3 2) (list 4 6 8 7)))
               2))
...
(define (double-list numbers)
  (if (null? numbers)
      ; What should this be?
      null
      ; Hint: construct a list out of the first part of the answer,
      ;       and use recursion for the rest of the answer.
      (let  ([first-num (first numbers)]
             [rest-doubled (double-list (rest numbers))])
        (append (list (* first-num 2)) rest-doubled))
      )
  )
...
(define (calculate expr)
  (cond
    [(null? expr) null]
    [(number? expr) expr]
    [else (let ([first-op (first expr)]
               [mid-expr (if (number? (second expr)) ;list-ref expr 1
                             (second expr)
                             (calculate (second expr)))]
               [end-expr (if (number? (third expr)) ;list-ref expr 2
                             (third expr)
                             (calculate (third expr)))])
          (cond
            [(eq? '+ first-op) (+ mid-expr end-expr)]
            [(eq? '- first-op) (- mid-expr end-expr)]
            [(eq? '* first-op) (* mid-expr end-expr)]
            [(eq? '/ first-op) (/ mid-expr end-expr)]
            ))
...
(module+ test
  ...
  (test-equal? "calculate: /"
               (calculate '(/ (+ 2 6) 2))
               4)
  (test-equal? "calculate: all"
               (calculate '(/ (+ 10 6) (- (* 4 (+ 1 3)) 8)))
               2))
...
(define (calculate-many exprs)
  (if (null? exprs)
      null
      (let ([first-expr (calculate (first exprs))]
            [rest-calc-expr (calculate-many (rest exprs))])
        (append (list first-expr) rest-calc-expr))
      )
  )

(module+ test
  (test-equal? "calculate-one:"
               (calculate-many '(13))
               '(13))
  (test-equal? "calculate-many:"
               (calculate-many '((+ 2 3) (/ (+ 2 6) 2))) ; '((+ 2 3) (/ 8 2)) is the same as
               '(5 4))
  (test-equal? "calculate-many-more:"
               (calculate-many '((* 2 3) (/ (+ 10 6) (- (* 4 4) 8)) 5))
               '(6 2 5)))